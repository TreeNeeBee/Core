...
namespace {
    ...
    inline MagicType makeBlockHeaderMagic() {
        return BLOCK_UNIT_MAGIC_BASE ^ MemoryManager::getRuntimeXorMask();
    }
...
PoolAllocator::~PoolAllocator() {
    ...
    if (pools_) {
        PoolsMap* map_ptr = pools_.release();
        if (map_ptr) {
            map_ptr->~PoolsMap();
            std::free(map_ptr);
        }
...
void PoolAllocator::initialize(UInt32 alignByte, UInt32 maxPoolCount) {
    ...
    void* raw_map = std::malloc(sizeof(PoolsMap));
    if (!raw_map) {
    ...
    try {
    // Use placement new to construct PoolsMap in pre-allocated memory
    PoolsMap* map_ptr = new (raw_map) PoolsMap();
        if (!map_ptr) {
            std::free(raw_map);
            raw_map = nullptr;
...
Bool PoolAllocator::createPool(UInt32 unitSize, UInt32 initCount, UInt32 maxCount, UInt32 appendCount) {
    CMutexGuard lock(&mutex_);
...
UInt32 PoolAllocator::getPoolCount() {
    CMutexGuard lock(&mutex_);
    return pools_ ? static_cast<UInt32>(pools_->size()) : 0u;
}

Bool PoolAllocator::getPoolState(UInt32 index, MemoryPoolState& poolState) {
    CMutexGuard lock(&mutex_);
...
void* PoolAllocator::malloc(Size size) {
...
void PoolAllocator::free(void* ptr) {
    ...
    if (!unit->pool) {
        ...
        if (unit->magic == SYSTEM_UNIT_MAGIC) {
            ...
            // Do NOT free it - it's either already freed (double-free) or never allocated by PoolAllocator
            // Use fprintf to stderr to avoid memory allocation in error path
            INNER_CORE_LOG("[ERROR] PoolAllocator::free: Invalid magic 0x%016lX at ptr=%p (expected SYSTEM_UNIT_MAGIC=0x%016lX). Possible double-free or memory not allocated by PoolAllocator.\n", 
    ...
    if (unit->magic != MAKE_UNIT_NODE_MAGIC(unit)) {
        ...
        INNER_CORE_LOG("[ERROR] PoolAllocator::free: Memory corruption detected at ptr=%p, magic=0x%016lX, expected=0x%016lX. Block not returned to pool to prevent further corruption.\n", 
...
Bool PoolAllocator::addPoolBlock(tagMemPool* pool) {
...
PoolAllocator::tagMemPool* PoolAllocator::findFitPool(Size size) {
...
void PoolAllocator::freeAllPool() {
    CMutexGuard lock(&mutex_);
...
const Int32 MemoryTracker::SIZE_INFO_MAX_COUNT;
    ...
    , classCount_(0)
{
    ...
    // Initialize pthread mutex for MemoryTracker
...
void MemoryTracker::initialize(Bool compactSizeRange, PoolAllocator* memAllocator) {
    CMutexGuard lock(&syncMutex_);
    compactSizeRange_ = compactSizeRange;
    poolAllocator_ = memAllocator;
    initSizeRange();
}

void* MemoryTracker::malloc(Size size, UInt32 classId) {
    CMutexGuard lock(&syncMutex_);
...
Int32 MemoryTracker::checkPtr(void* ptr, const Char* hint) {
...
UInt32 MemoryTracker::registerClassName(const Char* className)
{
    CMutexGuard lock(&syncMutex_);
...
void MemoryTracker::registerThreadName(UInt32 threadId, const String& threadName) {
    CMutexGuard lock(&syncMutex_);
...
void MemoryTracker::setReportFile(const String& reportFile) {
    CMutexGuard lock(&syncMutex_);
    std::strncpy(reportFile_, reportFile.c_str(), REPORT_FILE_MAX_LENGTH);
    reportFile_[REPORT_FILE_MAX_LENGTH] = '\0';
}

Bool MemoryTracker::outputState(UInt32 gpuMemorySize) {
    CMutexGuard lock(&syncMutex_);
    return outputStateToLogger(gpuMemorySize);
}

UInt32 MemoryTracker::getBlockExtSize() {
    return sizeof(tagBlockHeader);
}

UInt32 MemoryTracker::getThreadCount() {
    CMutexGuard lock(&syncMutex_);
    return threadCount_;
}

UInt32 MemoryTracker::getThreadID(UInt32 index) {
    CMutexGuard lock(&syncMutex_);
    return index < threadCount_ ? threadSizes_[index].threadId : 0;
}

Size MemoryTracker::getThreadSize(UInt32 index) {
    CMutexGuard lock(&syncMutex_);
...
void* MemoryTracker::hookMalloc(Size size, UInt32 classId) {
    Size totalSize = size + sizeof(tagBlockHeader);
    BlockHeaderPtr header = nullptr;
    Bool usedPool = false;
    if ( poolAllocator_ ) {
        header = reinterpret_cast<BlockHeaderPtr>( poolAllocator_->malloc(totalSize) );
        usedPool = true;
    } else {
...
void MemoryTracker::hookFree(void* ptr) {
    if (!ptr) return;
    BlockHeaderPtr header = reinterpret_cast<BlockHeaderPtr>(static_cast<char*>(ptr) - sizeof(tagBlockHeader));
    ...
    unlinkBlock(header);
       // When allocTag==1, header is the "user pointer" from PoolAllocator's perspective
       // PoolAllocator::free will subtract tagUnitNode size to find the actual allocation start
       if (poolAllocator_ && header->allocTag == 1u) {
           poolAllocator_->free(header);
       } else {
           ::std::free(header);
       }
}

MemoryTracker::EBlockStatus MemoryTracker::checkBlock(BlockHeaderPtr header) {
    if (!header) {
...
MemoryTracker::ELinkStatus MemoryTracker::checkAllBlock(UInt32& errorBlockCount) {
    CMutexGuard lock(&syncMutex_);
    errorBlockCount = 0;
...
void MemoryTracker::linkBlock(BlockHeaderPtr header) {
...
void MemoryTracker::unlinkBlock(BlockHeaderPtr header) {
    if (header->prev) {
...
Bool MemoryTracker::outputStateToLogger(UInt32 gpuMemorySize) {
...
void MemoryTracker::reportMemoryLeaks() {
    CMutexGuard lock(&syncMutex_);
...
void MemoryTracker::initSizeRange() {
...
Int32 MemoryTracker::calcRangeIndex(Size size) {
    if (compactSizeRange_) {
...
void MemoryTracker::logAllocSize(Size size) {
...
void MemoryTracker::logFreedSize(Size size) {
    (void)size; // Handled in unlinkBlock
}

void MemoryTracker::buildClassStat(MapThreadStat& threadStats) {
    CMutexGuard lock(&syncMutex_);
    for (BlockHeaderPtr p = blockList_; p; p = p->next) {
        const char* className = (p->classId == 0 || p->classId >= classCount_) 
                                ? "Global" 
...
MemoryManager::MemoryManager()
    ...
    , runtimeXorMask_(0)
{
    // Initialize pthread mutex for MemoryManager callbacks
    ::pthread_mutex_init(&callbackMutex_, nullptr);
    // Generate runtime XOR mask early in constructor
    generateRuntimeXorMask();
    
    void* allocBuf = std::malloc(sizeof(PoolAllocator));
    if (allocBuf) poolAllocator_ = new (allocBuf) PoolAllocator();

    // poolAllocator_->initialize(alignByte_, MAX_POOL_COUNT);
}

MemoryManager::~MemoryManager() {
    if (memoryTracker_) {
        memoryTracker_->~MemoryTracker();
        std::free(memoryTracker_);
    }
    if (poolAllocator_) {
        poolAllocator_->~PoolAllocator();
        std::free(poolAllocator_);
    }
...
void MemoryManager::initialize()
{
    ...
    PoolAllocator::PoolConfig localConfigs[MAX_POOL_CONFIG_ENTRIES];
    ...
    for (size_t i = 0; i < cfgCount; ++i) {
        ...
        (void)poolAllocator_->createPool(c.unitSize, c.initCount, c.maxCount, c.appendCount);
    }

    // Construct-only: enable checker when explicitly requested; do not destroy on false/missing
    if ( checkEnabled_ && !memoryTracker_ ) {
        void* checkerBuf = std::malloc(sizeof(MemoryTracker));
        if (checkerBuf) {
            memoryTracker_ = new (checkerBuf) MemoryTracker();
            memoryTracker_->initialize(true, poolAllocator_);
        }
    }
...
void MemoryManager::setListener(IMemListener* listener) { 
    CMutexGuard lock(&callbackMutex_);
    listener_ = listener;
}

UInt32 MemoryManager::getCurrentAllocCount()
{
    CMutexGuard lock(&callbackMutex_);
    if (!memoryTracker_) return 0u;
    return memoryTracker_->getCurrentAllocCount();
}

Size MemoryManager::getCurrentAllocSize() {
    
    CMutexGuard lock(&callbackMutex_);
    if (!memoryTracker_) return 0;
    return memoryTracker_->getCurrentAllocSize();
}

UInt32 MemoryManager::getThreadCount() {
    
    CMutexGuard lock(&callbackMutex_);
    if (!memoryTracker_) return 0u;
    return memoryTracker_->getThreadCount();
}

UInt32 MemoryManager::getThreadID(UInt32 index) {
    
    CMutexGuard lock(&callbackMutex_);
    if (!memoryTracker_) return 0u;
    return memoryTracker_->getThreadID(index);
}

Size MemoryManager::getThreadSize(UInt32 index) {
    
    CMutexGuard lock(&callbackMutex_);
    if (!memoryTracker_) return 0;
    return memoryTracker_->getThreadSize(index);
}
...
MemoryStats MemoryManager::getMemoryStats() {
    ...
    if (poolAllocator_) {
        stats.poolCount = poolAllocator_->getPoolCount();
        
        // Iterate through all pools to sum up memory usage
        for (UInt32 i = 0; i < stats.poolCount; ++i) {
            PoolAllocator::MemoryPoolState poolState;
            if (poolAllocator_->getPoolState(i, poolState)) {
    ...
    if (memoryTracker_) {
        stats.threadCount = memoryTracker_->getThreadCount();
    }
    
    return stats;
}

void* MemoryManager::malloc(Size size, const Char* className, UInt32 classId)
{
    // Fast path: check initialized without lock first
    // During pre-initialization (before MemoryManager::initialize completes),
    ...
    {
        if (memoryTracker_) {
            ptr = memoryTracker_->malloc(size, classId ? classId : className ? registerClassName(className) : 0);
        } else {
...
void MemoryManager::free(void* ptr) {
    ...
    if (memoryTracker_) {
        memoryTracker_->free(ptr);
    } else if (poolAllocator_) {
        poolAllocator_->free(ptr);
    } else {
        std::free(ptr);
    }
}

Int32 MemoryManager::checkPtr(void* ptr, const Char* hint) {
    
    CMutexGuard lock(&callbackMutex_);
    if (!memoryTracker_) {
        return 0; // No checking if checker is disabled
    }
    return memoryTracker_->checkPtr(ptr, hint);
}

UInt32 MemoryManager::registerClassName(const Char* className) {
    
    CMutexGuard lock(&callbackMutex_);
    if (!memoryTracker_) return 0u;
    return memoryTracker_->registerClassName(className);
}

Bool MemoryManager::outputState(UInt32 gpuMemorySize) {
    
    CMutexGuard lock(&callbackMutex_);
    if (!memoryTracker_) {
        return false;
    }
    return memoryTracker_->outputState(gpuMemorySize);
}

void MemoryManager::registerThreadName(UInt32 threadId, const String& threadName) {
    
    CMutexGuard lock(&callbackMutex_);
    if (memoryTracker_) {
        memoryTracker_->registerThreadName(threadId, threadName);
    }
}

Bool MemoryManager::savePoolConfig(const String& moduleName) {
    CMutexGuard lock(&callbackMutex_);
    ...
    try {
        ...
        config["check_enable"] = (memoryTracker_ != nullptr);
        
        // Build pools array
        nlohmann::json poolsArray = nlohmann::json::array();
        UInt32 poolCount = poolAllocator_->getPoolCount();
        for (UInt32 i = 0; i < poolCount; ++i) {
            PoolAllocator::MemoryPoolState st{};
            if (!poolAllocator_->getPoolState(i, st)) continue;
...
size_t MemoryManager::loadPoolConfig(PoolAllocator::PoolConfig* out, size_t maxCount, const String& fileName) {
    ...
    try {
        ...
        if ( DEF_LAP_IF_UNLIKELY( j.is_null() || j.empty() ) ) {
            ...
            INNER_CORE_LOG("MemoryManager: No 'memory' module config found, using defaults\n");
        } else {
            INNER_CORE_LOG("MemoryManager: Loaded configuration from ConfigManager module 'memory'\n");
        ...
        INNER_CORE_LOG("MemoryManager: Exception loading config, using defaults\n");
    }
...
void MemoryManager::generateRuntimeXorMask() {
    ...
    runtimeXorMask_ = mask;
        ...
        INNER_CORE_LOG("MemoryManager: Generated runtime XOR mask");
    #endif
}

MagicType MemoryManager::getRuntimeXorMask() noexcept {
