...
namespace ipc
{
                                                ...
                                                const SubscriberConfig& config) noexcept
    {
        ...
        while ( true ) {
            ...
            if ( DEF_LAP_IF_UNLIKELY( !subscriber_id_result ) ) {
                ...
                m_config.channel_id = subscriber_id_result.Value();
                
                // Get the allocated queue from shared memory
                ChannelQueue* queue = shm->GetChannelQueue( m_config.channel_id );
                if ( !queue ) {
                    return Result< Subscriber >( MakeErrorCode( CoreErrc::kIPCShmNotFound ) );
                }
                queue->STmin.store( m_config.STmin, std::memory_order_release );

                break;
            }
        }
        // Create subscriber
        Subscriber subscriber( m_config, std::move( shm ), 
                                std::move(allocator) );
        
        return Result< Subscriber >( std::move( subscriber ) );
    }

    Result< Subscriber > Subscriber::Create( int shm_fd,
                                           const SubscriberConfig& config ) noexcept
    {
        // Create shared memory manager
        auto shm = MakeUnique< SharedMemoryManager >();
    ...
    Result< Vector< Sample > > Subscriber::Receive( SubscribePolicy policy ) noexcept
    {
        ...
        for ( const auto& [_, read_channel] : m_readChannels[ m_iActiveChannelIdx.load( std::memory_order_acquire ) ] ) {
            auto read_result = read_channel->ReadWithPolicy( policy, m_config.timeout );
            if ( !read_result ) {
    ...
    void Subscriber::UpdateSTMin( UInt16 stmin ) noexcept
    {
        ...
        ChannelQueue* queue = m_pShm->GetChannelQueue( m_config.channel_id );
        if ( queue ) {
    ...
    {
        if ( !m_pShm || !m_pShm->GetControlBlock()->Validate() ) {
            return Result< void >( MakeErrorCode( CoreErrc::kIPCShmNotFound ) );
        }

        m_bConnected.store( true, std::memory_order_release );

        for ( const auto& [channel_id, _] : m_readChannels[ m_iActiveChannelIdx.load( std::memory_order_acquire ) ] ) {
            ChannelRegistry::ActiveChannel( m_pShm.get(), channel_id );
        }
    ...
    Result<void> Subscriber::Disconnect() noexcept
    {
        ...
        for ( const auto& [channel_id, _] : m_readChannels[ m_iActiveChannelIdx.load( std::memory_order_acquire ) ] ) {
            ChannelRegistry::DeactiveChannel( m_pShm.get(), channel_id );
        }

        return {};
    }

    Subscriber::Subscriber( const SubscriberConfig& config,
            ...
            , m_pAllocator(std::move(allocator))
    {
        ...
        INNER_CORE_LOG( "Subscriber::Subscriber - Created subscriber with ID: %u, type: %d\n", 
                            m_config.channel_id, 
        ...
        if ( ctrl->header.type == IPCType::kMPSC ) {
            ...
            m_readChannels[0].emplace( m_config.channel_id,
    ...
    Subscriber::Subscriber(Subscriber&& other) noexcept
    {
        ...
        m_bConnected.store( other.m_bConnected.load(std::memory_order_relaxed), std::memory_order_relaxed );

        m_bRunning.store( false, std::memory_order_relaxed );
        m_iActiveChannelIdx.store( 0, std::memory_order_relaxed );
        auto ctrl = m_pShm->GetControlBlock();

        INNER_CORE_LOG( "Subscriber::Subscriber - Move subscriber with ID: %u, type: %d\n", 
                            m_config.channel_id, 
                            static_cast<int>(ctrl->header.type) );
    ...
    Subscriber::~Subscriber() noexcept
    {
        ...
        if ( m_pShm ) {
            ...
            ChannelRegistry::UnregisterReadChannel( m_pShm->GetControlBlock(), m_config.channel_id );
            
            // ====== Step 3: Consume remaining messages in queue ======
            // This ensures ref_count is properly decremented
            
            for ( const auto& [channel_id, channel] : m_readChannels[ m_iActiveChannelIdx.load( std::memory_order_acquire ) ] ) {
                while ( true ) {
                    ...
                    if ( sample_result.HasValue() ) {
                        ...
                        if ( m_pEventHooks ) {
                            auto sample = std::move( sample_result ).Value();
                            m_pEventHooks->OnMessageReceived( sample.ChannelID(), sample.RawData(), sample.RawDataSize() );
                        }
        ...
        m_readChannels[0].clear();
        m_readChannels[1].clear();
    }
    ...
    void Subscriber::startScanner( UInt16 timeout_microseconds, UInt16 interval_microseconds ) noexcept
    {
        ...
        m_scannerThread = std::thread( &Subscriber::innerChannelScanner, this, timeout_microseconds, interval_microseconds );
    }
    ...
    void Subscriber::stopScanner() noexcept
    {
        ...
        WaitSetHelper::FutexWake( &m_pShm->GetControlBlock()->registry.write_seq, INT32_MAX );
    ...
    */
    void Subscriber::updateReadChannel( UInt64 write_mask ) noexcept
    {
        ...
        auto& read_channel = m_readChannels[ updateIndex ]; 
        ...
        auto is_connected = m_bConnected.load( std::memory_order_acquire );
        while ( write_mask ) {
            ...
            if ( queue != nullptr ) {
                ...
                if ( channel ) {
                    ...
                    if ( is_connected ) {
                        ChannelRegistry::ActiveChannel( m_pShm.get(), idx );
                    } else {
                        ChannelRegistry::DeactiveChannel( m_pShm.get(), idx );
                    }
                }
            }
    ...
    Result< Sample > Subscriber::innerReceive( UInt8 channel_id, SubscribePolicy policy ) noexcept
    {
        ...
        auto active = m_iActiveChannelIdx.load( std::memory_order_acquire );

        auto it = m_readChannels[ active ].find( channel_id );
        if ( it == m_readChannels[ active ].end() ) {
            return Result< Sample >( MakeErrorCode( CoreErrc::kChannelInvalid ) );
        }

        auto read_result = it->second->ReadWithPolicy( policy, m_config.timeout );
        if (!read_result) {
