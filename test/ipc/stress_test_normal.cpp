/**
 * @file        stress_test_normal.cpp
 * @brief       Stress test for NORMAL mode IPC
 * @details     High-frequency multi-process test with maximum subscribers
 *              - 30 subscribers (max for NORMAL)
 *              - 20000+ messages
...
constexpr const char* TEST_SHM_PATH = "/stress_test_normal";
constexpr int MESSAGE_COUNT = 20000;  // High volume for stress test
constexpr int SUBSCRIBER_COUNT = 30;  // NORMAL mode max
...
struct TestMessage : public Message
{
    ...
    TestMessage() : sequence(0), timestamp_ns(0), sender_id(0) {
...
void publisher_process()
{
    ...
    for (int i = 0; i < MESSAGE_COUNT; ++i) {
        ...
        msg->payload[0] = 'S';  // Simple marker instead of snprintf
...
void subscriber_process(int subscriber_id)
{
    ...
    while (received_count < MESSAGE_COUNT && timeout_count < 2000) {
    ...
    std::cout << "[Subscriber-" << subscriber_id << "] Completed" << std::endl;
    ...
    std::cout << "  - Gaps: " << sequence_gaps << std::endl;
    std::cout << "  - Latency (avg/p50/p99): " << avg_latency_us << "/" 
              << p50_latency_us << "/" << p99_latency_us << " μs" << std::endl;
    
    if (received_count < MESSAGE_COUNT * 0.90) {
        std::cout << "[Subscriber-" << subscriber_id << "] FAILED (< 90% received)" << std::endl;
        exit(1);
    }
}
...
{
    std::cout << "========================================" << std::endl;
    std::cout << "  NORMAL Mode Stress Test" << std::endl;
    std::cout << "========================================" << std::endl;
    ...
    std::cout << "\n========================================" << std::endl;
    if (failures == 0) {
        std::cout << "  ✓ NORMAL Stress Test PASSED" << std::endl;
        std::cout << "========================================" << std::endl;
        return 0;
    } else {
        std::cout << "  ✗ NORMAL Stress Test FAILED" << std::endl;
